...
pub(crate) fn selected_plugin_connector_snapshot<C>(
    ...
    let Some(selected_roots) = context
        .thread_init()
        .and_then(|thread_init| thread_init.get::<Vec<SelectedCapabilityRoot>>())
    else {
        return ConnectorSnapshot::default();
    };
    let Some(state) = context
        .thread_store()
        .and_then(|thread_store| thread_store.get::<SelectedExecutorPluginMcpState>())
    else {
        return ConnectorSnapshot::default();
    };
    let cache = state
...
impl McpServerContributor<Config> for SelectedExecutorPluginMcpContributor {
    ...
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            if let Some(selected_roots) = context
                .thread_init()
                .and_then(|thread_init| thread_init.get::<Vec<SelectedCapabilityRoot>>())
            {
