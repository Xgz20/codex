...
async fn stale_contributor_returns_last_good_and_refreshes_once_in_background() {
    ...
    service
        ...
        .refreshed_at = Some(std::time::Instant::now() - codex_connectors::CONNECTORS_CACHE_TTL);
    let mut thread_init = codex_extension_api::ExtensionDataInit::new();
...
async fn cold_contributor_coalesces_one_inventory_and_publishes_on_next_boundary() {
    ...
    let revision_before_waiter =
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref());
    let waiting = McpServerContributor::contribute(
        service.as_ref(),
    ...
    assert!(
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref())
            > revision_before_waiter,
        "a same-key waiter that missed the launch publication must get one wakeup"
    );

    let launch_revision = joining_thread_state.revision();
    ...
    tokio::time::timeout(std::time::Duration::from_secs(2), async {
        while joining_thread_state.revision() == launch_revision {
            tokio::task::yield_now().await;
        }
    })
    .await
    .expect("next boundary joins the in-flight connection");
    assert!(
        !join.is_finished(),
        "the joining boundary must wait for the shared inventory"
    );

    list_gate.add_permits(1);
    let contributions = tokio::time::timeout(std::time::Duration::from_secs(5), join)
        .await
        .expect("joined contribution completes")
        .expect("joined contribution task");
    assert_eq!(
        contributions.revision,
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref()),
        "a successful cold join must consume the reserved publication"
    );
    assert!(contributions.contributions.iter().any(|contribution| {
        matches!(
            contribution,
...
async fn cold_contributor_initializes_distinct_connection_keys_without_losing_publication() {
    ...
    let revision_after_a =
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref());
    assert!(revision_after_a > initial_revision);
    ...
    .await;
    assert_eq!(first_b.revision, revision_after_a);
    assert!(first_b.contributions.is_empty());
    let revision_after_b =
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref());
    assert!(revision_after_b > revision_after_a);
    assert!(
        service
            .connection
    ...
    assert_eq!(
        McpServerContributor::<codex_core::config::Config>::revision(service.as_ref()),
        revision_after_b,
        "successful discovery must not publish another revision"
    );
...
async fn failed_cold_initialization_retries_at_the_next_contribution_boundary() {
    ...
    let retry = McpServerContributor::contribute_with_revision(
        &service,
        McpServerContributionContext::for_thread(&config, &thread_init),
    )
    .await;
    assert_eq!(retry.revision, retry_revision);
    assert!(retry.contributions.iter().any(|contribution| {
        matches!(
            contribution,
            McpServerContribution::SetEffective { name, .. }
                if name == "codex_apps__gmail"
    ...
    let live_revision = McpServerContributor::<codex_core::config::Config>::revision(&service);
    assert_eq!(
        live_revision, retry_revision,
        "the successful retry consumes the publication captured by its joining boundary"
    );
    drop(retry);
    service.connection.clear_connected_through(u64::MAX);
    drop(service);
