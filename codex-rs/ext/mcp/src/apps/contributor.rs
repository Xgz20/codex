    ...
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            let (connection_key, apps, state_revision) = match apps {
                ...
                None => {
                    let Some((state, revision)) = thread_state.as_ref().zip(thread_state_revision)
                    else {
                    ...
                    match state.prepare_connection_if_revision(
                        ...
                        AppsConnectionPreparation::Launch { revision } => {
                            self.initialize_in_background(
                                config.clone(),
                                connection_key,
                        ...
                        AppsConnectionPreparation::Join { revision } => {
                            let publication_reservation =
                                self.connection.reserve_publication(connection_key.clone());
                            match self
                                .connection
                                .apps_for_config_with_reserved_publication(
                                    config,
                                    /*refresh*/ false,
            ...
            if let Some((state, revision)) = thread_state.as_ref().zip(state_revision) {
                if !state.replace_apps_if_revision(
                    revision,
                    connection_key,
                    Arc::clone(&apps),
                    snapshot.clone(),
