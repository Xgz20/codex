...
async fn concurrent_step_projections_keep_connector_attribution_disjoint() {
    let alpha = selected_root("alpha", "environment-alpha", "/plugins/alpha");
    let beta = selected_root("beta", "environment-beta", "/plugins/beta");
...
fn selected_root(id: &str, environment_id: &str, path: &str) -> SelectedCapabilityRoot {
    SelectedCapabilityRoot {
        id: id.to_string(),
        location: CapabilityRootLocation::Environment {
            environment_id: environment_id.to_string(),
            path: PathUri::from_host_native_path(Path::new(path)).expect("plugin root path URI"),
        },
    }
}
