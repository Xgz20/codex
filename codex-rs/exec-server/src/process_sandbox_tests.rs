...
#[cfg(target_os = "linux")]
#[test]
fn sandbox_request_forwards_sites_preview_to_linux_sandbox() {
    let cwd: AbsolutePathBuf = std::env::current_dir()
        .expect("current directory")
        .try_into()
        .expect("absolute cwd");
    ...
    let params = ExecParams {
        process_id: ProcessId::from("sites-preview"),
    ...
    let prepared = prepare_exec_request(&params, HashMap::new(), Some(&runtime_paths))
        .expect("prepare Sites preview sandbox request");

    assert!(prepared.command.iter().any(|arg| arg == "--sites-preview"));
}
