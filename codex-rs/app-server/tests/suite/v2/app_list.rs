...
async fn oauth_rejects_runtime_only_apps_server_before_network_discovery() -> Result<()> {
    ...
    .await??;
    let AppsListResponse { data, .. } = to_response(list_response)?;
    assert!(data.iter().any(|app| app.id == "beta"));

    let status_request = mcp
    ...
    let status_response = timeout(
        DEFAULT_TIMEOUT,
        mcp.read_stream_until_response_message(RequestId::Integer(status_request)),
    )
    .await??;
    let status_response: ListMcpServerStatusResponse = to_response(status_response)?;
    let beta_status = status_response
        .data
        .iter()
        .find(|status| status.name == "codex_apps__beta")
        .expect("the Apps connector must be live in the effective runtime view");
    assert_eq!(beta_status.auth_status, McpAuthStatus::BearerToken);
    assert!(
        !std::fs::read_to_string(codex_home.path().join("config.toml"))?
            .contains("codex_apps__beta"),
        "the loopback Apps endpoint must remain runtime-only"
    );
...
#[tokio::test]
async fn mcp_server_status_tracks_apps_namespace_replacement() -> Result<()> {
    let beta = AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: None,
