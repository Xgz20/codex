...
async fn virtual_server_uploads_from_a_pinned_replaced_environment() {
    ...
    let managed_permission_profile = PermissionProfile::from_runtime_permissions(
    ...
    #[cfg(unix)]
    let permission_profile = managed_permission_profile;
    #[cfg(not(unix))]
    let permission_profile = PermissionProfile::Disabled;
    let mut sandbox_state = SandboxState {
    ...
    let result = match upload {
        ...
        Err(error) => {
            ...
            if format!("{error:#}").contains("fs sandbox helper failed") {
                ...
                sandbox_state.permission_profile = PermissionProfile::Disabled;
                manager
                    .call_tool(
                        "codex_apps__gmail",
                        "upload",
