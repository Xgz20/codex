            ...
            move |Json(request): Json<Value>| {
                ...
                async move {
                    ...
                    Response::builder()
                        ...
                        .header(CONTENT_LENGTH, POST_RESPONSE_LIMIT + 1)
                        .body(Body::from_stream(stream::pending::<
                            Result<Bytes, Infallible>,
                        >()))
                        .expect("valid oversized initialize response")
                }
            }
        }),
    );
...
async fn handle_mcp_post(
    ...
    match method {
        ...
        _ => Response::builder()
            ...
            .body(Body::from("unexpected MCP method"))
...
fn json_response(value: Value) -> Response<Body> {
    Response::builder()
        ...
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(
            serde_json::to_vec(&value).expect("serialize JSON response"),
        ))
        .expect("valid JSON response")
}

fn chunked_response(value: Value, state: ServerState) -> Response<Body> {
    let json = serde_json::to_vec(&value).expect("serialize tools/list response");
