...
enum McpConfiguredBaseAuthority {
    /// The base is an intentional override, such as an ephemeral skill dependency snapshot.
    Independent,
    /// The base came from this session config and must be rebuilt if the session advances.
    SessionConfig(Arc<Config>),
}
...
impl Session {
    ...
    pub(crate) async fn mcp_runtime_for_step(
        ...
        );
        {
            ...
            let publication = self.services.mcp_runtime_publication.lock().await;
        ...
        let published_inputs = {
            ...
            Arc::ptr_eq(&publication.contributor_config, &contributor_config).then(|| {
                (
                    publication.configured_base.clone(),
    ...
    async fn refresh_mcp_servers_inner(
        ...
        configured_base_authority: McpConfiguredBaseAuthority,
    ) {
        let _refresh_guard = self.services.mcp_refresh_lock.lock().await;
        let contributor_config = self.mcp_source_config().await;
        let configured_base_is_stale = match &configured_base_authority {
            McpConfiguredBaseAuthority::Independent => false,
            McpConfiguredBaseAuthority::SessionConfig(expected) => {
                !Arc::ptr_eq(expected, &contributor_config)
            }
        };
        let (configured_base, store_mode, keyring_backend_kind) = if configured_base_is_stale {
            (
    ...
    async fn replace_mcp_servers(
        ...
        contributors_revision: crate::mcp::McpContributorsRevision,
        store_mode: OAuthCredentialsStoreMode,
        keyring_backend_kind: AuthKeyringBackendKind,
        ...
        *self.services.mcp_runtime_publication.lock().await = crate::state::McpRuntimePublication {
            contributor_config,
            configured_base,
            store_mode,
            keyring_backend_kind,
    ...
    pub(crate) async fn refresh_mcp_servers_if_requested(
        ...
        let (configured_base, store_mode, keyring_backend_kind, configured_base_authority) =
            match refresh {
                PendingMcpServerRefresh::Sourceful {
                    ...
                    store_mode,
                    keyring_backend_kind,
                    McpConfiguredBaseAuthority::SessionConfig(contributor_config),
                ),
                PendingMcpServerRefresh::SourceLess(refresh_config) => {
                    ...
                    let configured_servers = match serde_json::from_value::<
                        HashMap<String, McpServerConfig>,
                    >(mcp_servers)
                    {
                        Ok(servers) => servers,
                    ...
                    (
                        McpConfiguredBase::from_servers(configured_servers),
                        store_mode,
                        keyring_backend_kind,
                        McpConfiguredBaseAuthority::Independent,
                    )
                }
            };
    ...
    pub(crate) async fn set_openai_form_elicitation_support(
        ...
        let refresh = {
            let publication = self.services.mcp_runtime_publication.lock().await;
            PendingMcpServerRefresh::Sourceful {
                contributor_config: Arc::clone(&publication.contributor_config),
                configured_base: publication.configured_base.clone(),
                store_mode: publication.store_mode,
                keyring_backend_kind: publication.keyring_backend_kind,
            }
        };
        self.services
            .supports_openai_form_elicitation
    ...
    pub(crate) async fn refresh_mcp_servers_now_from_config(
        &self,
        turn_context: &TurnContext,
        config: Arc<Config>,
        elicitation_reviewer: Option<ElicitationReviewerHandle>,
        ...
        self.refresh_mcp_servers_inner(
            ...
            elicitation_reviewer,
            McpRefreshMode::Restart,
            McpConfiguredBaseAuthority::SessionConfig(config),
        )
        .await;
    }
