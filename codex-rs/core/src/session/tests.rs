...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let services = SessionServices {
        ...
        plugins_manager,
        mcp_manager,
        mcp_runtime_publication: Mutex::new(crate::state::McpRuntimePublication {
            contributor_config: Arc::clone(&config),
            configured_base: McpConfiguredBase::from_servers(config.mcp_servers.get().clone()),
            store_mode: config.mcp_oauth_credentials_store_mode,
            keyring_backend_kind: config.auth_keyring_backend_kind(),
            contributors_revision: Vec::new(),
            runtime_context: Session::mcp_runtime_context_for_environments(
...
async fn new_session_config_replaces_stale_queued_base_and_contributor_policy() {
    ...
    let publication = session.services.mcp_runtime_publication.lock().await;
    assert!(Arc::ptr_eq(
...
#[tokio::test]
async fn contributor_refresh_preserves_the_strictly_refreshed_mcp_base() {
    ...
    let replacement_server = test_http_mcp_server("http://127.0.0.1:9/mcp");
    let replacement_servers =
        HashMap::from([("replacement".to_string(), replacement_server.clone())]);
    ...
    refresh_config
        ...
        .expect("replacement MCP config");
    session
        .queue_mcp_server_refresh_from_config(&refresh_config)
        .await;
    session
        .refresh_mcp_servers_if_requested(&turn_context, /*elicitation_reviewer*/ None)
        .await;
